...
public class ProbeInsertion
{
    ...
    public ProbeInsertion(float ap, float ml, float depth, float phi, float theta, float spin)
    {
        SetCoordinates(ap, ml, depth, phi, theta, spin);
    }

    public ProbeInsertion(Vector3 coords, Vector3 angles, string transformString)
    {
    ...
    public void SetCoordinates(float ap, float ml, float depth, float phi, float theta, float spin)
    {
        this.ap = ap;
        this.ml = ml;
        this.depth = depth;
    ...
    {
        return string.Format("ccfAP:{1} ccfML:{2} ccfDV:{3} ccfPh:{4} ccfTh:{5} ccfSp:{6}", ap, ml, depth, phi, theta, spin);
    }
    ...
    {
        return (ap, ml, depth, phi, theta, spin);
    }

    public (Vector3, Vector3) GetCoordinatesVector3()
    {
        return (new Vector3(ap, ml, depth), new Vector3(phi, theta, spin));
    }

    /// <summary>
    /// Set coordinates in IBL conventions, expects um units and IBL rotated angles
    /// </summary>
    public void SetCoordinates_IBL(float ap, float ml, float depth, float phi, float theta, float spin)
    {
        Vector2 worldPhiTheta = IBL2World(new Vector2(phi, theta));
        SetCoordinates(ap / 1000f, ml / 1000f, depth / 1000f, worldPhiTheta.x, worldPhiTheta.y, spin);
    }
    ...
    public (float, float, float, float, float, float) GetCoordinatesFloat_IBL()
    {
        ...
        return (ap * 1000f, ml * 1000f, depth * 1000f, iblPhiTheta.x, iblPhiTheta.y, spin);
    }
